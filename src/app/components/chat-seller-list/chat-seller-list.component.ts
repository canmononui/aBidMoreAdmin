...
@Component({
  selector: 'app-chat-seller-list',
  templateUrl: './chat-seller-list.component.html',
  styleUrls: ['./chat-seller-list.component.css']
})
export class ChatSellerListComponent implements OnInit {
    ...
    .where(`readed.${adminUID}`, '==', false)
      ...
      .map(actions => {
        ...
        for (var i = 0; i < nonReadList.length; i++) {
          ...
          if (_members[0] != adminUID && nonReadList[i].value.members[_members[0]].type == 'seller') {
            ...
            if (nonReadList[i].value.members[_members[0]].profileImg.imgUrl == null) {
              nonReadList[i].value.members[_members[0]].profileImg.imgUrl = './assets/img/seller-profile/store-icon.png'
            }
            ...
            if (nonReadList[i].value.members[_members[1]].type == 'seller') {
              ...
              if (nonReadList[i].value.members[_members[1]].profileImg.imgUrl == null) {
                nonReadList[i].value.members[_members[1]].profileImg.imgUrl = './assets/img/seller-profile/store-icon.png'
              }
              nonReadList[i].value.chatTo = {
      ...
      .where(`readed.${adminUID}`, '==', true)
        ...
        .map(actions => {
          ...
          for (var i = 0; i < readedList.length; i++) {
            ...
            if (_members[0] != adminUID && readedList[i].value.members[_members[0]].type == 'seller') {
              ...
              if (readedList[i].value.members[_members[0]].profileImg.imgUrl == null) {
                readedList[i].value.members[_members[0]].profileImg.imgUrl = './assets/img/seller-profile/store-icon.png'
              }
              ...
              if (readedList[i].value.members[_members[1]].type == 'seller') {
                ...
                if (readedList[i].value.members[_members[1]].profileImg.imgUrl == null) {
                  readedList[i].value.members[_members[1]].profileImg.imgUrl = './assets/img/seller-profile/store-icon.png'
                }
                readedList[i].value.chatTo = {
  ...
  gotoChatRoom(chatKey, chatToUID, displayName) {
    this.router.navigate([`/chat-seller-message/${chatKey}&${chatToUID}&${displayName}`]);
  }
